...
import _root_.io.swagger.v3.core.util.{Json, Yaml}
import cats._
import org.http4s.headers.`Content-Type`
import org.http4s.rho.bits.PathAST.{PathMatch, TypedPath}
import org.http4s.rho.swagger.models.OpenAPI
import shapeless._
...
abstract class SwaggerSupport[F[_]: Monad](implicit etag: WeakTypeTag[F[_]])
  ...
  def createSwagger(
      swaggerFormats: SwaggerFormats = DefaultSwaggerFormats,
      swaggerMetadata: SwaggerMetadata = SwaggerMetadata(),
      showType: ShowType = DefaultShowType)(routes: Seq[RhoRoute[F, _]]): OpenAPI = {
